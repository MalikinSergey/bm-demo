        ...
        @foreach($families as $family)
            ...
            <tr>
                ...
                <td>
                    ...
                    @if($family->hasCover())
                        <div>
                            ...
                            <img src="{{$family->getCoverUrl()}}" alt="" style="width: 200px;"
                ...
                <td>
                    ...
                    {{$family->getPrice('personal')}} / {{$family->getPrice('commercial')}}
                    / {{$family->getPrice('commercial_ext')}}
                    <br>
                    <small>(updated: {{$family->updated_at->format('Y-m-d H:i')}})</small>
                ...
                <td>
                    <a href="{{route('dashboard.asset.edit_multiple', ['family_id' => $family->id])}}">{{$family->assets()->count()}}
                        assets</a>
                </td>

                <td>
                    <a href="{{route('dashboard.pack.index', ['family_id' => $family->id])}}">{{$family->packs()->count()}}
                        packs</a>
                </td>
                ...
                <td>
                    <a href="{{route('website.family.show', $family->slug)}}" target="_blank"
                       class="btn btn-sm btn-success">open</a>
                    {{--                    <a href="{{route('dashboard.family.show', $family)}}" class="btn btn-sm btn-primary">details</a>--}}
                    <a href="{{route('dashboard.asset.add_multiple', $family)}}" class="btn btn-sm btn-primary">upload
                        assets</a>
                    <a href="{{route('dashboard.family.edit', $family)}}" class="btn btn-sm btn-primary">edit family
                        info</a>
